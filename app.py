...
for k,v in txd.items():
    fig.add_trace(go.Scatter(x=preds['Month'].loc[preds['RegionName']==k], y=preds['MeanValue'].loc[preds['RegionName']==k], name=f'{k} actual', color='lightgrey', dash='solid'))
    fig.add_trace(go.Scatter(x=preds['Month'].loc[preds['RegionName']==k], y=preds['predicted'].loc[preds['RegionName']==k], name=f'{k} pred', color='royalblue', dash='solid'))
    fig.add_trace(go.Scatter(x=forecast['Month'].loc[forecast['RegionName']==k], y=forecast['predicted'].loc[forecast['RegionName']==k], name=f'{k} forecast', color='lightseagreen', dash='solid'))
